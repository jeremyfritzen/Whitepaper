...
1. Plusieurs connexions virtuelles (appelées "tunnels") sont établies entre l'utilisateur et chaque pair, afin d'améliorer l'efficacité des transmissions de données. La tentative de connexion se fait d'abord via les protocoles basés sur UDP tels que KCP ou UDT [13]. Si elle échoue, une tentative de connexion est réalisée via les protocoles TCP afin de s'adapter aux différents types de réseaux.
2. Pour chaque pair, une estimation de la vitesse de téléchargement, notée "$V_{conn}$", est calculée sur la base de son historique de transmission. Si aucun historique n'est disponible, une valeur empirique par défaut est utilisée.
3. Le nombre de connexions virtuelles pour chaque pair est variable. Un pair avec une valeur $V_{conn}$ plus élevée peuvent avoir un nombre important de connexions virtuelles.
4. En se basant sur les partitions du segment, l'utilisateur envoie d'abord une demande de téléchargement à un pair donné pour un morceau choisi arbitrairement. Le pair répond ensuite en envoyant à l'utilisateur le morceau demandé.
...
6. Quand une demande de téléchargement expire ("time-out"), la requête est annulée et toutes les connexions avec le pair indisponible sont fermées. Les requêtes de téléchargement des morceaux restants sont re-routées vers d'autres pairs. Dans ce cas, le pair indisponible sera aussi pénalisé et le nombre de connexions qui pourront être établies avec lui sera réduit.
...
Pour implémenter les prérequis de conception ci-dessus, un algorithme d'ordonnancement est nécessaire pour décider intelligemment depuis quel pair sera téléchargé chaque morceau, comment utiliser les pairs pour des téléchargements simultanés, le minuteur de chaque pair, replanifier le téléchargement des morceaux restants et s'adapter aux changements sur le réseau, etc. L'ordonnanceur est aussi conçu pour maximiser la vitesse de téléchargement et limiter les requêtes et la transmission de données en doublon. Voici un résumé de l'algoritheme d'ordonnancement :

1. Comme pour le téléchargement de fichiers, des connexions virtuelles (appelées "tunnels") sont établies avec chaque pair et la vitesse de transmission estimée ($V_{conn}$) est calculée pour chaque connexion virtuelle.
...
Le réseau de transmission P2P de PPIO est entièrement dynamique. Chaque pair répond à plusieurs demandes de téléchargement, et potentiellement à plusieurs noeuds de téléchargement. Chaque noeud de téléchargement envoie des requêtes à plusieurs pairs, gère les morceaux téléchargés et doit également gérer les potentielles expirations (timeouts) et échecs des pairs. Dans le même temps, le noeud de téléchargement peut lui-même répondre à des demandes de téléchargement, travaillant ainsi comme un pair pour les autres noeuds. En utilisant les 2 algorithmes d'ordonnancement orintés données, le réseau P2P dynamique de PPIO peut gérer un volume très important de téléchargements de données simultanés avec une grande efficacité.
...
### 3.2.2. Base de données distribuée et routage

PPIO est système de stockage et de distribution entièrement décentralisé qui nécessite une base de données qui doit également être gérée de façon décentralisée. PPIO utilise une Table de Hachage Distribuée ("DHT" ou "Distributed Hash Table") pour stocker et fournir des services de routage et de recherche.

Dans une Table de Hachage Distribuée, à chaque bout ou morceau de donnée est associé une clé unique grâce à une pair "clé-valeur". Ces pairs "clé-valeur" sont distribuées et stockées au sein des noeuds. Cela permet d'évitdwidth from the data centers. As the number of users grows, their transfer speed and user experience will likely degrade.er les problèmes rencontrés avec une base de données centralisée, où les pannes sur un serveur central provoquent une indisponibilité du réseau tout entier. Dans une Table de Hachage Distribuée, aucun noeud n'a à maintenir seul la totalité du réseau. Chaque noeud stocke seulement une petite partie de la base de données et doit seulement maintenir les informations des noeuds adjacents. Une telle conception réduit significativement l'usage de la bande passante et des ressources de chaque noeud. La redondance des informations est aussi ajoutée aux données et stockée sur plusieurs noeuds afin d'assurer la fiabilité du système de sorte que la panne d'un seul noeud ne cause aucune perte de données.

Il existe de nombreuses implémentations différentes pour mettre en place une Table de Hachage Dsitribuée. Parmi les plus répandues, on retrouve les implémentations de type "Chord"[14], "Patry"[15], "Tapestry"[18], "Dynamo"[19] ou encore "Kademlia"[16]. PPIO utilise Kademlia, tout comme de nombreux autres projets P2P connus tels que PPLive, BitTorrent, eMule, etc.

L'algorithme Kademlia assigne un ID unique à chaque noeud participant. L'ID de chaque noeud est alors utilisé pour calculer la distance entre les noeuds et pour localiser les paires "clé-valeur". L'algorithme traverse le réseau et trouve des noeuds plus proches de la clé à chaque étape pour chercher un couple clé-valeur spécifique sur le réseau jusqu'à ce qu'il atteigne le noeud qui stocke la clé-valeur. Chaque noeud doit stocker des informations sur un groupe de noeuds, incluant sa distance avec chacun d'entre eux, afin de pouvoir faire des recherches. Cette distance est calculée sur la base d'une opération XOR réalisée sur 2 ID de noeuds. Les ID ont la même taille de bits que la clé utilisée dans le couple clé-valeur. Ainsi, la distance entre les IDs de noeuds et les clés peut être calculée de la même façon. Puisque la distance calculée avec des opérations XOR ne s'appuie pas sur la distance géographique, il est possible que 2 noeuds soient définis comme proches sur le réseau alors que l'un d'eux se trouve au Royaume-Uni et l'autre aux Etats-Unis.

![kad_find_example](images/kad_find_example.png)

PPIO adopte aussi une extension de Kademlia - S/Kademlia[23] pour améliorer la sécurité sur le réseau. Ces changements sont principalement causés par des noeuds via une recherche parallèle sur des chemins disjoints. Le réseau peut supporter des attaques même si un nombre important de noeuds a été compromis.

Avec l'implémentation DHT décrite ci-dessus, les données structurées commes les index, les statistiques sur les noeuds du réseau et d'autres méta-données peuvent être sauvegardées de façon sécurisée et rapidement accessibles sur le réseau distribué PPIO. Ceci étant dit, le transfert de données est géré de façon différente. C'est ce que nous décrivons dans la prochaine section.

### 3.2.3. P2P Self-organizing Overlay networks

Today’s well-established storage services, such as Amazon S3, Dropbox, Google Drive, iCloud, etc., store customer data in large-scale data centers. Under cost constraints, each of the services can only deploy a limited number of data centers around the world. The deployment of Amazon AWS data centers is shown in Figure 3.3. Due to the ad hoc nature of the Internet, it is challenging to guarantee fast data transfer everywhere in the world by relying on a handful of data centers. Users in the same region have to compete for the available ban
